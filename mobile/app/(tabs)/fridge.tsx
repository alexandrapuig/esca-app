...
export default function FridgeScreen() {
  ...
  const loadItems = useCallback(async () => {
    ...
    if (response.success && response.data) {
      const active = response.data.filter((item) => item.storage_location !== "pantry");
      setItems(active);
      await storageService.saveFridgeItems(active);
  ...
  const emptyState = useMemo(
    () => (
      <View style={styles.emptyState}>
        <Text style={styles.emptyText}>Your fridge is empty. Add your first item.</Text>
      </View>
    ),
    []
  );
